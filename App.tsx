          ...
          {view === 'manageAccounts' && (
            ...
            <View style={{ margin:16, backgroundColor:'#1c2128', borderRadius:14, overflow:'hidden' }}>
              ...
              <TouchableOpacity onPress={()=>{ Alert.alert('Remove Wallet','Are you sure?',[{text:'Cancel'},{text:'Remove',style:'destructive',onPress:onRemoveWallet}]); }}
                ...
                <Text style={{ color:'red',flex:1,fontSize:15 }}>Remove Wallet</Text>
                <Text style={{ color:C.muted }}>›</Text>
              </TouchableOpacity>
            </View>
            </ScrollView>
...
function TokenModal({ token, pubkey, onClose }: { token: any, pubkey: string | null, onClose: () => void }) {
  if (!token) return null;
  const usdVal = token.price && token.amount ? (token.price * token.amount).toFixed(2) : null;
  return (
    <Modal visible={!!token} animationType="slide" transparent onRequestClose={onClose}>
      <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.6)', justifyContent:'flex-end' }}>
        <View style={{ backgroundColor:'#161b22', borderTopLeftRadius:24, borderTopRightRadius:24, padding:24 }}>
          <View style={{ flexDirection:'row', alignItems:'center', marginBottom:20 }}>
            <View style={{ flex:1 }}>
              <Text style={{ color:C.text, fontWeight:'bold', fontSize:20 }}>{token.symbol}</Text>
              <Text style={{ color:C.muted, fontSize:13, marginTop:2 }}>{token.amount?.toFixed(4)} tokens</Text>
              {usdVal && <Text style={{ color:C.green, fontSize:13, marginTop:2 }}>${usdVal}</Text>}
            </View>
            <TouchableOpacity onPress={onClose} style={{ padding:8 }}>
              <Text style={{ color:C.muted, fontSize:22 }}>×</Text>
            </TouchableOpacity>
          </View>
          <Text style={{ color:C.muted, fontSize:11, fontFamily:'monospace', marginBottom:20 }}>{token.mint}</Text>
          <TouchableOpacity onPress={onClose} style={{ backgroundColor:C.green, borderRadius:14, padding:16, alignItems:'center' }}>
            <Text style={{ color:'#0d1117', fontWeight:'bold', fontSize:15 }}>Close</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
    ...
    <SafeAreaView style={s.root}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity style={[s.walletBtn, wallet ? s.walletBtnOn : null]} onPress={() => setShowWalletModal(true)}>
          <Text style={[s.walletBtnTxt, wallet ? { color: C.green } : null]}>{wallet ? wallet.slice(0,4)+'...'+wallet.slice(-4) : 'Connect Wallet'}</Text>
        </TouchableOpacity>
      </View>
